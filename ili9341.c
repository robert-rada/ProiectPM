...
{
	DDRB |=(1<<4)|(1<<2)|(1<<3)|(1<<5);//CS,SS,MOSI,SCK as output(although SS will be unused throughout the program)
	SPCR0=(1<<SPE0)|(1<<MSTR0);//mode 0,fosc/4
	SPSR0 |=(1<<SPI2X0);//doubling spi speed.i.e final spi speed-fosc/2
	PORTB |=(1<<4);//cs off during startup
}
...
void ili9341_spi_send(unsigned char spi_data)//send spi data to display
{
	...
	SPDR0=spi_data;//move data into spdr
}
